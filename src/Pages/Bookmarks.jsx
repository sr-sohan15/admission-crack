...
const Bookmarks = () => {
  ...
  const allQuestions = [
    ...admissionData.science.subjects.flatMap(s => s.questions),
    ...admissionData.arts.subjects.flatMap(s => s.questions)
  ];
  ...
  const removeBookmark = (qId) => {
    ...
    toast.info('রিভিশন তালিকা থেকে সরানো হয়েছে');
  };
    ...
    <div className="space-y-6 max-w-4xl w-full mx-auto py-2">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs sm:text-sm mt-1">পরীক্ষার আগের রাতে শুধু এই পয়েন্টগুলো রিভিশন দিয়ে যাও</p>
        </div>
      ...
      {bookmarkedQuestions.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-8 sm:p-12 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            যেকোনো বিষয়ের প্রশ্ন পড়ার সময় উপরে বুকমার্ক আইকনে ক্লিক করে এখানে সেভ করে রাখতে পারো।
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {bookmarkedQuestions.map((q, idx) => (
            <div key={q.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-4 sm:p-7 space-y-4 shadow-sm">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3 text-xs">
                <span className="bg-slate-800 text-slate-300 px-2.5 py-1 rounded-lg font-bold">{q.type}</span>
                <button 
              ...
              <div className="p-4 bg-emerald-950/30 border border-emerald-500/30 rounded-2xl space-y-2 text-xs text-slate-300">
                <p className="font-bold text-emerald-400 text-xs sm:text-sm">{q.hack}</p>
                <p className="text-amber-300 font-semibold">{q.eliminationTip}</p>
